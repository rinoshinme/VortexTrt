...
namespace vortex
{
    AlexNet::AlexNet(const std::string& model_path)
    {
        // load data from file
        std::vector<unsigned char> engine_data;
        loadBinaryContent(model_path, engine_data);
    ...
    void AlexNet::Preprocess(cv::Mat& image)
    {
        // preprocess
        cv::Mat temp;
    ...
    void AlexNet::Infer(cv::Mat& image, std::vector<float>& output)
    {
